...
// Log all Bond films released on odd years
const oddBonds = [];
for (let films of bondFilms) {
    if (films.year % 2 !== 0) {
        oddBonds.push(films);
...
// Sum up the total gross of all Bond movies throughout the years
const bondGrossArr = [];
for (let films of bondFilms) {
    let removedDollarSign = films.gross.replace("$", "");
    let removedComma = removedDollarSign.replace(/,/g, "");
    let convertedNumber = Number(removedComma);
    bondGrossArr.push(convertedNumber);
}
let totalGross = 0;
for (let value of bondGrossArr) {
